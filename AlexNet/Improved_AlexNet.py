...
# ==========全局设置==========
# 配置阿里云CIFAR-10镜像源，优先级最高，彻底替换默认国外源
os.environ['TORCHVISION_DATASETS_CIFAR10_URL'] = 'https://mirrors.aliyun.com/pytorch-vision/cifar-10-python.tar.gz'
np.random.seed(42)
...
# ==========模型初始化==========
model = Improved_AlexNet().to(device) # GPU加速
...
def visualize(train_loss, test_loss, train_acc, test_acc):
    ...
    plt.tight_layout()
    fig_name = "Improved_AlexNet.png"
    fig_path = os.path.join("D:/medical_image_process/AlexNet/figure", fig_name)
    plt.savefig(fig_path, dpi=150, bbox_inches='tight', facecolor='white', edgecolor='none',
                pad_inches=0.1, format='png', transparent=False)
...
def main():
    ...
    train_dataset = datasets.CIFAR10(
        root="D:\medical_image_process\data", train=True, transform=transform_train, download=False
    )
    test_dataset = datasets.CIFAR10(
        root="D:\medical_image_process\data", train=False, transform=transform_test, download=False
    )
    # 数据加载器
